...
/**
 * Group vertical slides according to max num unique tags: bounded greedy search
 * Group slides according to best pairwise scores: bounded greedy search
 *
 *
 */
public class SlideAlgorithm21 implements SlideAlgorithm {

    public static final int WINDOW_SIZE = 5000;

    public final PictureInput input;

    public SlideAlgorithm21(PictureInput input) {
        this.input = input;
    }
    ...
    private List<Slide2> processVerticalPhotos() {
        ...
        List<Picture> verticalPictures = input.pictures.stream()
                .filter(picture -> !picture.isHorizontal)
                .collect(Collectors.toList());
        Collections.shuffle(verticalPictures, new Random(0));

        while (verticalPictures.size() > 1){

            Picture firstPic = verticalPictures.remove(0);
            int stoppingIndex = Math.min(WINDOW_SIZE, verticalPictures.size());

            int bestPicIndex = 0;
            int bestPicScore = 0;
            for (int i = 0; i < stoppingIndex; i++) {

                Picture currPic = verticalPictures.get(i);
                int currScore = Slide2.numUniqueTags(firstPic, currPic);
                if(currScore >= bestPicScore){
                    bestPicScore = currScore;
                    bestPicIndex = i;
                }
            }

            Picture bestPic = verticalPictures.remove(bestPicIndex);
            verticalSlides.add(new Slide2(firstPic, bestPic));
        }
